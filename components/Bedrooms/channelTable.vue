    ...
    <table class="w-full ">
      ...
      <thead class="sticky top-0 ">
        <tr class="bg-white ">
          ...
          <th class="px-8 py-6 text-sm font-semibold ">
            <div class=" table-type text-center">
              OTA <br> Close
            </div>
          </th>
      ...
      <tbody class="overflow-y-auto h-[calc(100vh-(150px))] ">
        <tr v-for="(e, index) in dataTable" :key="e" class="font-semibold px-2 row-table"
          ...
          <td class="px-8 text-center whitespace-nowrap text-sm font-semibold data-table">
            <select v-model="selected" class="w-full text-center rounded-xl bg-gray-200 border border-gray-400">
          ...
          <template>
            <td class="border-l border-gray-300 px-2 font-semibold data-table" v-for="day in days" :key="day">
              <div class="flex justify-between items-center text-xs font-semibold ">
                ...
                <span v-if="e.Status === false" class="rounded-full bg-red-500 p-1 text-center text-red-400">.</span>
